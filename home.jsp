...
/* 		.swiper22{background: url(${stax}/assets/images/swiper8.jpg) no-repeat 50% top;background-size: contain;} */
		...
		.swiper26{background: url(${stax}/assets/images/20190520_bannerTuan.jpg) no-repeat 50% top;background-size: contain;}
		...
		.yurenjie{background: url(${stax}/assets/images/fool/yurenjiebanner.jpg) no-repeat 50% top;background-size: contain;}
...
#local-special .weui-media-box_appmsg h4{font-size: .7rem;}
    ...
    .restalert,.ginfoalert{width: 70%;height:55%;background: #fff;position: absolute;top: 20%;left: 10%;z-index: 1000;border-radius: 5px;padding:1rem 1rem;}
...
#willopen .weui-media-box__hd img{    border-radius: 0;
   ...
   .sign1 , .sign2 ,.sign3{width:100%;height:100%;position:fixed;top:0;background:rgba(0,0,0,0.5);z-index:1111;}	
	...
	.user{font-size:0.24rem;color:#C93E39;background:#F2D1C2;padding:1%;width:3rem;border-radius:0.5rem;font-weight:bold;text-align:center;position:absolute;top:0.1rem;letter-spacing:0.1rem;right:1rem;}
	.user .userDiv{border:1px solid #C93E39;border-radius:0.5rem;font-size:0.24rem;}
	.user .userDiv p{font-size:0.5rem;color:#C93E39;}
	.userCont p{font-size:0.56rem;color:#fff;}
	.userCont{display:none;background:rgba(0,0,0,0.5);font-size:0.24rem;position:absolute;top:0.2rem;left:3rem;color:#fff;text-align:start;padding:2%;border-radius:0.2rem;z-index:11;margin-right:4.6rem;font-family:"方正黑体简体"}
...
<!--          <p style="font-size:.8rem;text-align:center;margin-bottom:10px;">元旦节放假通知</p> -->
	 尊敬的客户：<br />
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;您好！<br />
&nbsp;&nbsp;&nbsp;&nbsp;因系统升级原因，“天津惠”平台将于2019年6月1日-6月30日暂停服务，预计将于7月开放，给您带来的不便敬请您谅解。您持有的到期日为2019年6月1日（含）之后的津元宝，津元宝使用到期日统一延长30天。建议您2019年5月31日前，在“天津惠”个人中心里，查看津元宝余额，并通过“天津惠”惠享商城、兑奖专区、本地特惠，使用津元宝，感谢您关注“天津惠”，天津惠客服：400-810-9200。<br />
			...
			<div class="c-content" style="padding-bottom: 20px;">
				...
				<div class="swiper-container" id="mySwiper">
					...
					<div class="swiper-wrapper" id="lbtimg">
						<!-- Slides -->
						<div class="swiper-slide yurenjie"><label>入金</label></div>
						...
						<div class="weui-flex__item">
							<a href="javascript:void(0);" class="weui-media-box weui-media-box_appmsg" id="c-a1">
								<div class="weui-media-box__bd">
									<h4 class="weui-media-box__title">幸运锦鲤</h4>
									<p class="weui-media-box__desc">竞猜金价赢锦鲤</p>
								</div>
								<div class="weui-media-box__hd">
									<img class="weui-media-box__thumb" src="${stax}/assets/images/xyjl20190408.png" alt="">
								</div>
							</a>
						</div>
						<div class="weui-flex__item">
				...
				<div class="c-index-hot"  style="position: relative;">
					...
					<h3>
						<img class="c-remenhd c-m-r-5" src="${stax}/assets/images/shopList-02.png">惠享商城
						<span class="c-fr" style="display: none;">更多</span>
					</h3>
					<div class="int yzh" >
						<img class="c-remenhd c-m-r-5" src="${stax}/assets/images/yzhbanner.jpg">
					</div>	
				</div>
				<div class="c-index-hot" id="xinyongka" style="margin-top:7px">
	    	...
	    	$.ajax({
	    		...
	    		success:function(resp){
	    			...
	    			if(images.length > 0){
	    				...
	    				$("#mySwiper .swiper-slide:nth-of-type(2)").after(img)
...
//         });
	  ...
	  //信用卡专区
		...
		$(".swiper26").on("click",function(){
			...
			var date2 = new Date("2019/05/17 00:00:10").getTime()
			if(date < date2){
				weui.alert("本期拼团有礼活动已结束，第二期拼团有礼活动即将开始，敬请期待。");
				return ;
			}
...
// 	  		$("#iosDialog3").hide();
	...
	function closeFun(){
		...
		if(msg == null){
			...
			var setDate1= new Date("2019/05/20 08:00:00").getTime();
			var setDate2= new Date("2019/06/01 23:59:00").getTime();
			if(setDate1 <= myDate){
...
//     			console.log('banner log done');
	...
	/* 炒股大赛 */
	var rjDate =  new Date("2019/06/12 17:00:00").getTime(); //入金时间
	if(Date.now() >= rjDate){
		console.log(123);
		$(".yurenjie label").hide();
	}else{
		$(".yurenjie label").show();
		$(".yurenjie label").click(function(e){
			e.stopPropagation();
			goPage("rujin");
		})
	}
