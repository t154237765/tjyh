...
<head>
    ...
    <style type="text/css">
        ...
        .openbtn{line-height: 2.5rem;width:2.8rem;height:1.8rem;background:url(${stax}/assets/images/goldbig.jpg) no-repeat;background-size:contain;color:#b86c00;font-size: .6rem;text-align: center;vertical-align: middle;    position: absolute;bottom: 1rem;left: 3.3rem;}
    ...
    function fillPrd(data){
        ...
        for(var i=0; i<data.length; i++){
            ...
            pdiv += '<div class="weui-flex__item">'+
                            '<a href="javascript:void(0);" class="weui-media-box weui-media-box_appmsg">'+
                                '<div class="weui-media-box__bd">'+
                                    '<div class="weui-media-box__hd touchView" data-pid="'+data[i].id+'">'+
                                        '<img class="weui-media-box__thumb" src="'+pimg+'" alt="">'+
                                    '</div>'+
                                    '<h4 class="weui-media-box__title am-text-truncate">'+data[i].prodName+'</h4>'+
                                    
                                    '<p class="pri c-f-s-16"><span class="c-c-r">￥'+data[i].prodPrice+'</span><br/><span class="c-f-s-12 c-oldprice">￥'+data[i].oldPrice+'</span></p>'+
                                    '<p class="weui-media-box__desc">已售<span>'+data[i].saled+'份</span></p>'+
                                    /* '<div class="openbtn touchView" data-pid="'+data[i].id+'">198</div>'+ */
                                '</div>'+
                            '</a>'+
                        '</div>';
            if(i % 2 == 0){
...
//              cango = true;
        ...
        iscrollObj.destroy();
        iscrollObj = null;
        window.location.href = "${ctx}/pintuan/pdetail.do?bankId="+bankId+"&pid="+pid;
    }
