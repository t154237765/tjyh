...
gulp.task('copy', function() {
  return gulp.src('amazeui.chosen*.js')
    .pipe(gulp.dest('dist'));
});
...
gulp.task('docs', function(){
  return gulp.src(['README.md', 'docs/*.md'])
    ...
    .pipe(docUtil.applyTemplate(null, {
      pluginTitle: 'Amaze UI Chosen',
      pluginDesc: '使用 Amaze UI 样式风格的 jQuery Chosen 插件。',
      buttons: 'amazeui/chosen',
      head: '<link rel="stylesheet" href="../amazeui.chosen.css"/>'
    }))
...
gulp.task('less', function() {
  return gulp.src('src/amazeui.chosen.less')
    .pipe($.less())
