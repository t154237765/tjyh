...
<head>
    ...
    <style type="text/css">
        ...
        #scroll-bottom-block{width:100%;height:45px;background:#F9CD82;}
        ...
        #wrapper{background:#F9CD82;}
        .weui-cells:before,.weui-cells:after{border: 0;}

        /*新首页*/
        #dataArea{background:url(${stax}/assets/images/zhuanxiang_03.jpg) no-repeat #F9CD82;width: 100%;background-size:contain;min-height:14rem;}
        .c-top-bg{background:url(${stax}/assets/images/zhuanxiang_02.jpg) no-repeat;width: 100%;background-size:cover;height:240px;}
        .c-bottom-bg{background:url(${stax}/assets/images/zhuanxiang_04.jpg) no-repeat;width: 100%;background-size:cover;height:120px;}
        /*内容*/
        ...
        .weui-media-box_appmsg .weui-media-box__hd{width:100%;height:5rem;display:flex;align-items:center;}
        .weui-media-box_appmsg h4, .c-goods-details h4.weui-media-box__title{font-size: .7rem;text-align:center;}
    	...
    	.c-a{font-size:0.56rem; text-align:center;display:block;margin-top:10px;}
...
<body style="position: relative;">
    ...
    <div class="rulealert" style="display: none;">
        ...
        <div class="text1">
&nbsp;&nbsp;1.本活动具备参与资格用户以各个支行提交的客户名单为准。<br />
&nbsp;&nbsp;2.本活动用户参与次数和限制以各个支行制定规则为准。<br />
&nbsp;&nbsp;3.本活动中提供商品均为活动专享，不支持退订及开发票，购买时敬请注意。<br />
&nbsp;&nbsp;4.开团24小时内达到规定的成团人数，即可拼团成功，系统发货。用户可在【个人中心】——【我的专享活动】中查看相关信息。<br />
&nbsp;&nbsp;5.开团24小时内未能达到相应人数参团，该团失败，系统自动退款。<br />
&nbsp;&nbsp;6.开团或参团后请于3分钟内完成支付，否则系统将自动删除订单，支付无效。<br />
&nbsp;&nbsp;7.本活动仅限使用建行手机银行APP龙支付，不支持建行账号支付方式。<br />
&nbsp;&nbsp;8.活动声明：活动过程中，不允许使用虚拟手机号注册天津惠并参加活动，参与者如出现作弊行为（如批量注册、虚假交易、恶意套利等）或使用外挂、插件及其他破坏活动规则，违背活动公平的方式，主办方有权取消您的交易资格及作弊订单，且有权取消您后续参与任意活动的权利，并追究法律责任。<br />
&nbsp;&nbsp;9.商品问题客服电话：4008109200，支付问题客服电话：022-58750200，咨询时间：周一至周五9:30至17:00。<br />
        </div>
    </div>
...
&nbsp;&nbsp;预祝各位国庆节假期愉快，期待大家持续关注我们的活动，你的参与就是对我们最大的支持！<br />
    ...
    <div class="container">
        <div class="titleBar">
            ...
            <div class="titleBar-text am-text-truncate">专享活动</div>
        </div>
                    ...
                    <div>
                        <div class="c-top-bg" style="position: relative;">
                            ...
                            <div class="weui-search-bar" id="searchBar">
                                <form class="weui-search-bar__form" id="toSearch" style="display:none;">
                            ...
                            <div class="weui-search-bar" id="ruleblock" style="display:none;">
...
<!--                             </div> -->
                    ...
                    <div id="scroll-bottom-block" style="background:#F9CD82"></div>
                </div>
            </div>
        </div>
...
// localStorage.clear();
    ...
    function getProds(iscrollObj){
        ...
        function promiseLoad(){
            return new Promise(function(resolve,reject){
                $.ajax({
                    type: "GET",
                    url: "${ctx}/huituan/getProdInfo.json",
                    data: "bankId="+encodeURIComponent(bankId)+"&lastId="+lastId+"&uid="+encodeURIComponent(uid)+"&r="+Math.random(),
    ...
    function fillPrd(data){
        ...
        for(var i=0; i<data.length; i++){
            ...
            var saledAndRemain = '';
            var actiCode = data[i].actiCode;
            if(status != undefined && status == 4){
            	saledAndRemain = '<p class="pri c-f-s-16"><span class="c-c-r c-a">'+data[i].prodName+'</span></p>';
            	saledAndRemain = 
            		'<div class="weui-flex__item">'+
	                    '<a href="javascript:void(0);" class="weui-media-box weui-media-box_appmsg">'+
	                        '<div class="weui-media-box__bd">'+
	                        '<div class="weui-media-box__hd touchView" data-pid="'+data[i].id+'" data-property="'+data[i].property+'" data-status="'+data[i].status+'">'+
	                            ...
	                            '</div>'+
	                            '<p class="pri c-f-s-16"><span class="c-c-r c-a">'+data[i].prodName+'</span></p>'+
	                            '<p class="pri c-f-s-16" style="text-align:center;">￥0.04</p>'+
	                        '</div>'+
	                    '</a>'+
             		'</div>';
            }else if(actiCode != undefined && actiCode == "webdzp"){
            	saledAndRemain = 
            		'<div class="weui-flex__item">'+
	                    '<a href="javascript:void(0);" class="weui-media-box weui-media-box_appmsg">'+
	                        '<div class="weui-media-box__bd">'+
	                        '<div class="weui-media-box__hd touchView" data-acticode="'+data[i].actiCode+'" data-pid="'+data[i].id+'" data-property="'+data[i].property+'" data-status="'+data[i].status+'">'+
	                    ...
	                    '</a>'+
             		'</div>';
            }else if(actiCode != undefined && actiCode == "orgtest"){
            	saledAndRemain = 
            		'<div class="weui-flex__item">'+
	                    '<a href="javascript:void(0);" class="weui-media-box weui-media-box_appmsg">'+
	                        '<div class="weui-media-box__bd">'+
	                        '<div class="weui-media-box__hd touchView" data-acticode="'+data[i].actiCode+'" data-pid="'+data[i].id+'" data-property="'+data[i].property+'" data-status="'+data[i].status+'">'+
	                            ...
	                            '</div>'+
	                            '<h4 class="weui-media-box__title am-text-truncate">'+data[i].prodName+'</h4>'+
	                            '<div class="openbtn touchView" data-pid="'+data[i].id+'">去答题</div>'+
	                        '</div>'+
	                    '</a>'+
             		'</div>';
            }else{
        ...
        $(".touchView").on("tap",function(){
            toView(this.dataset.pid, this.dataset.property,this.dataset.status ,this.dataset.acticode);
        });
    }

    
    //点击图片查看商品详情页面
    function toView(pid, property,status,acticode){
        /* if(pid == "16"){
...
//              cango = true;
    	...
    	console.log("status="+acticode);
    	//校验status=4，跳转暖冬页面
    	if(status == 4){
    		window.location.href ="${ctx}/huituan/nuandongactivity.do?uid="+encodeURIComponent(uid)+"&pid="+pid;
    	}else if(acticode =="webdzp"){
    		window.location.href ="${ctx}/netsilver/tjhpage.do";
    	}else if(acticode =="orgtest"){
    		window.location.href ="${ctx}/orgtest/index.do?uid="+encodeURIComponent(uid);
    	}else{
    	 	var loading = weui.loading('正在加载...');
	        iscrollObj.destroy();
	        iscrollObj = null;
	     	window.location.href = "${ctx}/huituan/pdetail.do?bankId="+encodeURIComponent(bankId)+"&pid="+pid+"&uid="+encodeURIComponent(uid);	
    	}
    }
    ...
    $("#toSearch").on("touchstart",function(){
       // if(!checkUser()) return false;
        //if(checkLocation()){
            ...
            iscrollObj.destroy();
            iscrollObj = null;
            window.location.href = "${ctx}/huituan/search.do";
       // }
    });
...
//       			window.location.href = '${ctx}/mycenter/personal.do' + (localStorage.getItem("userId") ? "?userId="+encodeURIComponent(localStorage.getItem("userId")) : "");
    ...
    function checkWhiteList(pid) {
    	...
    	$.ajax({
			type: "GET",
			url: "${ctx}/huituan/checkwl.json",
			data: "uid="+encodeURIComponent(uid)+"&pid="+pid,
			async:false,
			success: function(resp){
				res = resp.include;
				/* if (resp.wphone) {
...
//          return false;
   ...
   /*  function gohx() {
		...
		$.ajax({
    		type: "GET",
    		url: "${ctx}/huituan/checkhxwl.json",
    		...
    		success: function(resp){
        		...
        		if(resp.needInput){
        			...
        			window.location.href = '${ctx}/huituan/indexhx.do?uid='+localStorage.getItem("tjuserId");
        		} else {
   ...
   /*  function checkIdCard() {
        ...
        $.ajax({
            type: "GET",
            url: "${ctx}/huituan/saveAndCheckIdCard.json",
