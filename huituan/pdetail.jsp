...
<!--                     </div> -->
                    ...
                    <div class="weui-flex c-goods-details">
                        <div class="c-list-left-div">
                            ...
                            <ul class="weui-media-box__info" style="display:none;">
...
<!--             </div> -->
        ...
        <%-- <div class="page__bd" id="twoTab">
            <div class="weui-tab">
                <div class="weui-navbar weui-navbar-down">
                    ...
                    <div class="weui-navbar__item blue" id="viewTuanDiv">
						别人在开团
                    </div>
                </div>
            </div>
        ...
        <div class="page__bd" id="twoTab">
            <div class="weui-tab">
                <div class="weui-navbar weui-navbar-down">
                    <div class="weui-navbar__item red">
                    	立即购买
                    </div>
                </div>
            </div>
...
<!--                         <span style="vertical-align: middle;font-size: 15px;">现金券</span> -->
<!--                     </div> -->
...
// 	var discountList = null;
    ...
    $(document).ready(function() {
	    ...
	    $('#viewTuanDiv').on('click', function(){
	    	...
	    	window.location.href = "${ctx}/huituan/tuanlist.do?pid=${prod.id}";
	    });
    ...
    function toView(pid, tcode){
    	...
    	var loading = weui.loading('正在加载...');	//loading
    	window.location.href = "${ctx}/huituan/sdetail.do?bankId="+encodeURIComponent(bankId)+"&pid="+pid+"&tcode="+tcode;
    }
	...
	function getProdDetail(){
		...
		$.ajax({
			type: "GET",
			url: "${ctx}/huituan/getBuyInfo.json",
		...
		if(!isNaN(prodNum)){
			if(parseInt(prodNum) > 0){
				$.ajax({
					...
					data: "detailId="+$('#detailId').val()+"&pid=${prod.id}"+"&bflag=1"+"&userId="+encodeURIComponent(userId)+"&userwphone="+localStorage.getItem("tjuserWPhone")+"&addrId="+addrId+"&bankId="+encodeURIComponent(bankId)
...
// 	}
	...
	function delAddre(obj){
		...
		weui.confirm('确定删除？', function(){
			...
			if(addressList && addressList.length > parseInt(ind)){
				...
				$.ajax({
					type: "POST",
					url: "${ctx}/huiaddr/delAddr.json",
					data: "aid="+aid+"&uid="+encodeURIComponent(userId)+"&r="+Math.random(),
					success: function(resp){
						loading.hide();
						console.log(resp);
						if(resp.status){
	...
	function addAddress(){
		...
		$.ajax({
			type: "POST",
			url: "${ctx}/huiaddr/addAddr.json",
			data: "uid="+encodeURIComponent(userId)+"&recName="+$('#addName').val()+"&phone="+$('#addPhone').val()+"&address="+$('#textarea').val()
...
//			f = Math.round(x*100)/100;
    ...
    /**返回首页**/
 	/* function toHome(){
    	if (actiCode == 'hx_branch') {
    		window.location.href = '${ctx}/huituan/indexhx.do?uid='+userId;
    	} else {
