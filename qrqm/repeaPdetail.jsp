...
<body>
	...
	<div class="container">
		<div class="titleBar" id="dingdan-title">
			...
			<jsp:include page='../qrqm/childPages/${prodCode}.jsp'></jsp:include>
		</div>
		...
		<div class="weui-tab">
			<div class="weui-navbar weui-navbar-down">
				<img src="../assets/images/buynow.png" class="c-buynow"
					id="buyButton" /><!-- onclick="buyButton();" -->
		...
		<div class="weui-dialog">
			...
			<div class="weui-dialog__ft c-btn-noline" style="padding-top: 10px;">
				<a href="javascript:;"
					...
					style="line-height: 1.4rem; height: 1.4rem; flex: none;" onclick="dialogbtn();">绑定</a>
			</div>
		</div>
	</div>
	...
	<script type="text/javascript">
		...
		//验签
		/* var ccbtimestamp ="${ccbtimestamp}";
		if(ccbtimestamp){
			localStorage.setItem("ccbtimestamp", ccbtimestamp);
		}
		var ccbsign ="${ccbsign}";
		if(ccbsign){
			localStorage.setItem("ccbsign", ccbsign);
		}
		var checkBegin = "${checkBegin}";
		if(checkBegin != ''){
			weui.alert(checkBegin);
			$("#buyButton").removeAttr("onclick");
			$(".weui-navbar__item").removeAttr("onclick");
			$(".weui-dialog__btn").removeAttr("onclick");
			$(".titleBar-back").removeAttr("onclick");
		}  */
		$(function() {
			$.ajax({
						type : "GET",
						url : "${ctx}/repeaqrqm/showDetail.json",
		...
		function buyButton() {
			$.ajax({
				type : "GET",
				url : "${ctx}/repeaqrqm/checkUserLogin.json",
				data : "bid="
		...
		function getCode() {
			...
			oBtn.onclick = function() {
				if (bClick == false && checkPhone($("#phone").val())) {
					...
					$.ajax({
						type : "GET",
						url : "${ctx}/repeaqrqm/qrqmGetCode.json",
		...
		function register() {
			...
			$.ajax({
				type : "GET",
				url : "${ctx}/repeaqrqm/checkQRQMCode.json",
		...
		function checkBuyNum(){
			...
			$.ajax({
				type : "POST",
				url : "${ctx}/repeaqrqm/checkBuyNum.json",
...
// 		}
	...
	function MBS_DIRECT_PAY(){
		...
		function toHome(){
			$.ajax({
				type: "GET",
				url: "${ctx}/repeaqrqm/gotoDetailsPage.json",
				data: "bid="+encodeURIComponent(localStorage.getItem("bid"))+"&token="+encodeURIComponent(localStorage.getItem("token")),
				success: function(resp){
