        ...
        <div class="page__bd" id="twoTab">
            <div class="weui-tab">
                <div class="weui-navbar weui-navbar-down weui-navbar-right">
                    ...
                    <div class="red c-f-s-18 weui-navbar-div checkBuy">
						一键参团
                    </div>
                </div>
            </div>
		...
		if(!isNaN(prodNum)){
			if(parseInt(prodNum) > 0){
				$.ajax({
					...
					data: "detailId="+$('#detailId').val()+"&pid=${prod.id}"+"&tuanId=${tuan.id}"+"&userId="+encodeURIComponent(userId)+"&userwphone="+localStorage.getItem("tjuserWPhone")+"&addrId="+addrId+"&bankId="+encodeURIComponent(bankId)
