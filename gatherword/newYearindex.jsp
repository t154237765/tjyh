    ...
    <link rel="stylesheet" href="${stax}/assets/css/weui.min.css" />
  	<style type="text/css">
        *{
        ...
        .weui-mask, .weui-mask_transparent{width:100%;height:100%;}
        .box{
        	background:url(${stax}/assets/images/jiziBack1.jpg) no-repeat 100% 100%;
		...
		.jizi{width:7.3rem;padding:2% 0%;background:url(${stax}/assets/images/jizi1.png) no-repeat 100% 100%;background-size:100% 100%;height:5.2rem;position:absolute;top:5.2rem;right:0;left:0;margin:auto;}
		...
		.jizi p{font-size:0.28rem;color:black;text-align:center;margin-top:0.2rem;}
		.jizi p span{color:#b40117;}
...
<body>
    <div class="box">
		...
		<div class="jizi">
			...
			<div>
				<img src="${stax}/assets/images/jiziBtn1.png" id="gwBtn"/>
			</div>
			<p>今天剩余参与次数：<span class="remainNums">4次</span></p>
			<p style="font-size:.24rem">随机支付0.1-1元，体验支付，实时退款，集字得大奖！</p>
		</div>
		<div class="prize">
			<div class="img">
				<img src="${stax}/assets/images/prize1.png"/>	
			</div>
			<div class="prizeCont">
				<ol>
					<li>
						<p><span>特等奖：</span>建行天津惠祝您鼠年吉祥</p>
						<div>3888</div>
					</li>
					<li>
						<p><span>一等奖：</span>天津惠祝您鼠年吉祥</p>
						<div>1000</div>
					</li>
					<li>
						<p><span>二等奖：</span>祝您鼠年吉祥</p>
						<div>200</div>
					</li>
					<li>
						<p><span>三等奖：</span>鼠年吉祥</p>
						<div>50</div>
					</li>
					<li>
		...
		<div class="rule">
			<div class="img">
				<img src="${stax}/assets/images/rule1.png" />	
			</div>
			<div class="ruleCont">
				1、活动时间：2020年1月22日-2020年2月29日。 <br>
				2、活动内容： <br>
				（1）活动期间每位用户每天可以参与4次； <br>
				（2）支付一次可获得一次抽取机会，支付金额0.1-1元金额随机，体验支付，实时退款（储蓄卡：实时退回；信用卡：支付次日起1-2个工作日退回）；<br>
				（3）每笔支付，无论成功，均扣减一点体力值，如未支付成功，抽奖次数将会在三分钟后退回。 <br>
				（4）活动开始后每日00:00-00:10进行系统维护，暂时不可参与，维护结束活动可正常参与。<br>
				3、中奖规则 <br>
				（1）特等奖：成功集字“建行天津惠祝您鼠年吉祥”，奖励津元宝3888锭，一等奖：成功集字“天津惠祝您鼠年吉祥”，奖励津元宝1000锭，二等奖：成功集字“祝您鼠年吉祥”，奖励津元宝200锭，三等奖：成功集字“鼠年吉祥”，奖励津元宝50锭，参与奖：集字成功但未能组成“建行天津惠祝您鼠年吉祥”、“天津惠祝您鼠年吉祥”、“祝您鼠年吉祥”、“鼠年吉祥”其中一句，无论集字字数，均只奖励津元宝1锭。 <br>
...
/***********************************************************加载页面信息***********************************************************/
	...
	if(acid) localStorage.setItem("gwacid", acid);   
	if(decd) localStorage.setItem("cd", decd);    		    		
	if(detm) localStorage.setItem("tm", detm);    		    		
	if(demc) localStorage.setItem("mc", demc);   
	if(bid)localStorage.setItem("bankuid", bid);    	
	
	$(function(){
		var log = new Log("/recordlog.gif");
		if(modelType&&modelType=="qrqm"){
			log.write(localStorage.getItem("bankuid"), "cjjizi", "index_qrqm");
		}else{
			log.write(localStorage.getItem("bankuid"), "cjjizi", "index_tjh");
		}
		//加载用户信息
	...
	function getbuyerinfos(){
		$.ajax({
			url:'${ctx}/gatherwordvert/getbuyerinfos.json',
			type:'post',
			data:'userId='+encodeURIComponent(localStorage.getItem("userId"))
			+'&acid='+encodeURIComponent(localStorage.getItem("gwacid"))+'&r='+Math.random(),
			success:function(resp){
				if(resp.buyerInit && resp.buyerInit ){
					var buyer = resp.buyerInit;
					if(buyer.pm1>0) {
						$(".pm1").addClass("on");
						$(".pm_end").addClass("on");
					}
				...
				if(resp.noLuckyDraw&&resp.noLuckyDraw=="1"){
					isLuckyDraw = false;
					play(resp.orderNo);
				}else{
					isLuckyDraw = true;
				}
				if(resp.remainNums && resp.remainNums!=""){
...
/***********************************************************去集字***********************************************************/
	...
	function cucgw(){
		$.ajax({
			url:'${ctx}/gatherwordvert/cucgw.json',
			type:'post',
			data:"userId=" + encodeURIComponent(localStorage.getItem("userId"))+"&acid="+encodeURIComponent(localStorage.getItem("gwacid"))+'&r='+Math.random(),
			success:function(resp){
				if(resp.msg!=""){
	...
	var oos = "";
	function play(orderNo){
		var loading = weui.loading('正在加载...');
		$.ajax({
			type : "POST",
			url : "${ctx}/gatherwordvert/play.json",
			data : "userId=" + encodeURIComponent(localStorage.getItem("userId"))+"&acid="+encodeURIComponent(localStorage.getItem("gwacid"))+"&orderNo="+orderNo+"&bid="+localStorage.getItem("bankuid")+
			"&modelType="+modelType+"&cd="+localStorage.getItem("cd")+"&tm="+localStorage.getItem("tm")+"&mc="+localStorage.getItem("mc")+"&r="+Math.random(),
			success : function(resp) {
				loading.hide();
				isLuckyDraw = true;
				if (resp.msg != ""){
	...
	function register() {
		...
		if (!ccode) {
			...
			$.ajax({
				...
				data : "phone="+phone+"&code="+ccode +"&bid="+localStorage.getItem("bankuid")+"&bankId="+encodeURIComponent("${bankId}")+"&acid="+encodeURIComponent(localStorage.getItem("gwacid"))
