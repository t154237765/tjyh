		...
		<div class="weui-dialog" style="max-width:335px;">
			...
			<div class="weui-dialog__bd c-f-s-16" style="color: #333; padding-top: 1.7em;">
				...
				<div class="weui-cells weui-cells_form">
					...
					<div class="weui-cell" style="margin-top:0.2rem;">
						<div class="weui-cell__bd">
							<input id="phone2" class="weui-input" type="tel" maxlength="11"
								pattern="[0-9]*" placeholder="请填写邀请人手机号（可不填）">
						</div>
					</div>
					<p style="color:red;font-size:0.26rem;margin-top:0.1rem;">填写邀请人后不可更改哟</p>
				</div>
			</div>
...
/***********************************************************加载页面信息***********************************************************/
	...
	var orderNo = '';
	$(function(){
		if(buyerId && buyerId != '') getbuyerinfos();
		else{
...
$(".confirm2").click(function(){
	...
	function goBuy() {
		...
		$.ajax({
			...
			data : "buyerId=" + encodeURIComponent(buyerId)+"&type=1&r="+Math.random(),
			success : function(resp) {
				loading.hide();
				if (resp.msg != ""){
					$(".err-tishi p").html(resp.msg);
	...
	function getCode() {
		...
		oBtn.onclick = function() {
			if (bClick == false && checkPhone($("#phone").val())) {
				...
				$.ajax({
					type : "GET",
					url : "${ctx}/gatherword/getcode.json",
	...
	function register() {
		...
		if (!ccode) {
			...
			$.ajax({
				type : "GET",
				url : "${ctx}/gatherword/register.json",
				data : "phone="+phone+"&code="+ccode +"&bid="+"${bid}"+"&bankId="+encodeURIComponent("${bankId}")+"&activityId="+encodeURIComponent("${activityId}")+"&iuphone="+iuphone+"&r=" + Math.random(),
				success : function(resp) {
					$("#iosDialog1").hide();
