...
<script>
	...
	function toBuy(){
		...
		$.ajax({
			...
			data: "pid="+encodeURIComponent(pid)+"&bankid="+bankId
			+"&userId="+userId + "&userphone="+encodeURIComponent(localStorage.getItem("userPhone"))+"&r="+Math.random(),
	        	...
	        	$(".reg_input button").click(function(){
		        		...
		        		weui.confirm("您确定"+accountT+"为油卡登记信息吗，一经绑定天津惠内通用且无法修改，不再提示。", function() {
		        			  ...
		        			  $.ajax({
		        				  ...
		        				  type:'post',
		        				  data:'accountF='+accountF+"&accountT="+accountT+"&uid="+userId+"&r="+Math.random(),
		        				  success:function(resp){
