...
<body>
    ...
    <div class="container">
        <div class="titleBar">
            ...
            <div class="titleBar-text am-text-truncate">我的专享活动</div>
        </div>
	...
	function getOrders(iscrollObj){
        ...
        function promiseLoad(){
            return new Promise(function(resolve,reject){
                $.ajax({
                    type: "GET",
                    url: "${ctx}/mycenter/gettuan.json",
                    data: "userId="+encodeURIComponent(userId)+'&bankuid='+encodeURIComponent(localStorage.getItem("bankuid"))+"&start="+lastId+"&length=10&r="+Math.random(),
                    success: function(resp){
                        if(lastId == 0 && !resp.orderList.length){
                            $("#wrapper").html("<img src='${stax}/assets/images/null.png' style='margin:0 auto;height:4.5rem;margin-bottom:1.5rem;'><span style='font-size:.8rem;color:#999;'>您还没有参与支行活动哦</span>").css({"paddingTop":"6rem"});
	...
	function fillPrd(data){
    	for(var i=0; i<data.length; i++){
			...
			if(tuanStatus == "1"){
				...
				statusText = "已成团";
				specialDiv = '<p class="weui-media-box__desc">购买日期:<span>'+data[i].buildTime+'</span></p>';
			...
			var pdiv = '<div href="javascript:void(0);" class="weui-media-box weui-media-box_appmsg c-m-b-10" dataid="'+data[i].id+'" datasta="'+tuanStatus+'">'+
					        ...
					        '<div class="weui-media-box__bd">'+
					            ...
					            '<p class="weui-media-box__desc"><span class="c-c-r c-m-r-5">￥ '+data[i].payMoney+'</span>原价 <s>'+data[i].oldPrice+'</s></p>'+
					            '<p class="weui-media-box__desc">拼团ID：'+(data[i].tuanCode ? data[i].tuanCode : '')+'</p>'+
					            specialDiv+
    		...
    		$('.weui-media-box_appmsg').on('tap', function(){
    			...
    			if(oid){
    				if("2" == $(this).attr('datasta')){
    					window.location.href = '${ctx}/mycenter/tuanorderdetail.do?id='+oid;
    				}else if("1" == $(this).attr('datasta')){
    					window.location.href = '${ctx}/pay/pageBack.do?REMARK1=HPT&REMARK2='+oid;
    				}
    			}
    		});
    	}
