...
<head>
    ...
    <style type="text/css">
        ...
        .openbtn{line-height: 2.2rem;width:2.2rem;height:1.6rem;background:url(${stax}/assets/images/goldbig.jpg) no-repeat;background-size:contain;color:#b86c00;font-size: .6rem;text-align: center;vertical-align: middle;    position: absolute;bottom: 2.5rem;left: 9.1rem;}
        ...
        .Hui-iconfont-arrow2-left{font-size:0.6rem;}
        .exam{position:absolute;top:4.2rem;right:1rem;}
        .exam p{width:3rem;padding:2% 0%;text-align:center;color:#fff;border-radius:0.2rem;background:deepskyblue;}
    </style>
    <title></title>
</head>
<body>
    ...
    <div class="container">
        <div class="titleBar">
            ...
            <div class="titleBar-text am-text-truncate">我的兑换</div>
        </div>
...
<script type="text/javascript" >
    ...
    function getOrders(iscrollObj){
        ...
        function promiseLoad(){
            return new Promise(function(resolve,reject){
                $.ajax({
                    type: "GET",
                    url: "${ctx}/mycenter/getmyexchange.json",
                    data: "userId="+encodeURIComponent(userId)+'&bankuid='+encodeURIComponent(localStorage.getItem("bankuid"))+"&start="+startIndex+"&r="+Math.random(),
                    success: function(resp){
                        if(page == 1 && !resp.orderList.length){
                            $("#wrapper").html("<img src='${stax}/assets/images/null.png' style='margin:0 auto;height:4.5rem;margin-bottom:1.5rem;'><span style='font-size:.8rem;color:#999;'>您还没有兑换哦</span>").css({"paddingTop":"6rem"});
    ...
    function fillPrd(data){
    	for(var i=0; i<data.length; i++){
			var pdiv = '<div href="javascript:void(0);" class="weui-media-box weui-media-box_appmsg c-m-b-10" dataid="' + encodeURIComponent(data[i].id) + '">'+
	                        '<div class="weui-media-box__hd">'+
	                            '<img class="weui-media-box__thumb" src="${stax}/'+data[i].picUrl+'" alt="">'+
	                        '</div>'+
	                        '<div class="weui-media-box__bd">'+
	                            '<h4 class="weui-media-box__title">'+data[i].prodName+'</h4>'+
	                            '<p class="weui-media-box__desc c-c-r">¥'+data[i].payMoney+'+<br/>'+
	                            '<p class="c-f-s-xs c-oldprice">¥'+ data[i].oldPrice + '</p>'+
	                        '</div>'+
	                        '<div class="c-change">'+
	                        ...
	                        '</div>'+
	                        '<div class="openbtn touchView" data-pid="4">' + data[i].ingotNum + '</div>'+
	                        '<div class="exam"><p>查看详情</p></div>'+
	                    '</div>'
    	...
    	if (data.length) {
        	$('.exam').on('tap', function(){
        		var eid = $(this).parents(".weui-media-box_appmsg").attr('dataid');
        		window.location.href = '${ctx}/mycenter/exchangedetail.do?id=' + eid + '&userId=' + encodeURIComponent(localStorage.getItem("userId"))
        	});
    	}
    }
</script>
